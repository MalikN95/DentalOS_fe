...
- `/reviews` — owner/admin-only moderation screen (`ReviewsPageContent` + `ReviewsTable`) over the backend's `reviews` module (`GET/PATCH /api/reviews`, previously registered but with no frontend at all). One row per review: date, patient, doctor, star rating (0 = requested but not yet rated by the patient), comment, a status badge (`pending`/`published`/`hidden`, filterable), a `SwitchToggle` to mark it **featured** (curated for the future public landing page — only allowed on `published` reviews; unpublishing a review clears its `featured` flag), and a Publish/Hide action button. `useReviews` (React Query, `useInvoices`-style stateful pagination+filters) backs both this page and the two `ReviewsCard` read-only views above; `useUpdateReviewStatus`/`useUpdateReviewFeatured` are the two mutations. `ReviewEntity` gained a `featured` column this round (migration `ReviewsFeatured...`, alongside a first migration for the `reviews` table itself, which — like `doctor_services` earlier — had only ever existed via `DB_SYNC` in dev, never in a migration).
