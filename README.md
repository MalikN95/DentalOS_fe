...
- Root-domain routing (`src/middleware.ts`): the bare app domain (no clinic subdomain — e.g. `localhost:3000`, as opposed to `maximum.localhost:3000`) is rewritten at `/` to the marketing landing page (`src/app/marketing`) instead of the clinic dashboard/login. Detection reuses `extractSubdomainFromHostname` (`helpers/tenant.ts`); any `*.APP_DOMAIN` subdomain is untouched and keeps today's behavior (`/` → dashboard, guarded by `useRequireAuth`). Only the exact `/` path is rewritten — every other route (`/login`, `/book`, etc.) is unaffected on the bare domain.
...
- Public booking widget (`/book`) — the doctor-selection step shows a `★ X.X (N)` rating summary per doctor (`BookingDoctorStep`), computed backend-side (`BookingService.getRatingSummaries`) from that doctor's `showInBooking` reviews only; doctors with no qualifying reviews show no rating line.
